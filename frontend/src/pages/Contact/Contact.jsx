        ...
        {on && !hasErr && (
          <motion.div
            ...
            style={{
              ...
              background: 'linear-gradient(90deg,transparent,#E4F141,transparent)',
  ...
  return (
    <section ref={ref} className="page-shell contact-page-shell" style={{ position: 'relative', overflow: 'hidden', paddingBottom: '3rem' }}>
      ...
      <div style={{
        ...
        backgroundImage: 'linear-gradient(rgba(228,241,65,0.06) 1px,transparent 1px),linear-gradient(90deg,rgba(228,241,65,0.06) 1px,transparent 1px)',
      ...
      <motion.div animate={{ scale:[1,1.2,1], opacity:[0.1,0.2,0.1] }} transition={{ duration:8, repeat:Infinity, ease:'easeInOut' }}
        style={{ position:'absolute', top:'-12%', left:'28%', width:'700px', height:'580px', background:'radial-gradient(ellipse,rgba(228,241,65,0.22) 0%,transparent 65%)', pointerEvents:'none', zIndex:0 }} />
      <motion.div animate={{ scale:[1,1.15,1], opacity:[0.05,0.12,0.05] }} transition={{ duration:11, repeat:Infinity, ease:'easeInOut', delay:4 }}
        style={{ position:'absolute', bottom:'-8%', right:'-4%', width:'480px', height:'380px', background:'radial-gradient(ellipse,rgba(228,241,65,0.15) 0%,transparent 70%)', pointerEvents:'none', zIndex:0 }} />
          ...
          >
                  ...
                  <AnimatePresence>
                    {hoveredLine === i && (
                      <motion.div
                        ...
                        style={{ position:'absolute', left:0, top:'10%', width:'3px', height:'80%', background:'#E4F141', borderRadius:'2px', transformOrigin:'center' }}
                      />
                    )}
                  </AnimatePresence>
                  <motion.span
                    animate={{
                      color: hoveredLine===i ? (line.accent?'#E4F141':'#fff') : (line.accent?'rgba(228,241,65,0.85)':'rgba(255,255,255,0.25)'),
                      textShadow: hoveredLine===i ? (line.accent?'0 0 25px rgba(228,241,65,0.6)':'0 0 14px rgba(255,255,255,0.12)') : 'none',
                    }}
                    transition={{ duration:0.18 }}
            ...
            >
              {contactDetails.map((item,i) => (
                ...
                >
                  <div style={{ width:'32px', height:'32px', borderRadius:'8px', background:'rgba(228,241,65,0.12)', border:'1px solid rgba(228,241,65,0.4)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'0.9rem', flexShrink:0, boxShadow:'0 0 15px rgba(228,241,65,0.2)' }}>
                    {ICONS[item.label]||'📌'}
                  </div>
                  <div>
            ...
            <form onSubmit={handleSubmit} noValidate style={{ display:'flex', flexDirection:'column', gap:'0.75rem' }}>
              ...
              <div style={{ position:'relative' }}>
                <textarea name="message" rows={6}
                  ...
                  style={{
                    width:'100%', padding:'13px 16px',
                    background: errors.message?'rgba(255,69,0,0.06)': focused==='message'?'rgba(228,241,65,0.1)':'rgba(255,255,255,0.04)',
                    border:`1px solid ${errors.message?'rgba(255,69,0,0.55)':focused==='message'?'rgba(228,241,65,0.6)':'rgba(255,255,255,0.1)'}`,
                    borderRadius:'12px', color:'#fff', fontSize:'0.88rem',
                    outline:'none', resize:'vertical', boxSizing:'border-box',
                    fontFamily:'inherit', lineHeight:1.65,
                    transition:'border-color 0.2s,background 0.2s,box-shadow 0.2s',
                    boxShadow: errors.message?'0 0 0 3px rgba(255,69,0,0.12)':focused==='message'?'0 0 0 3px rgba(228,241,65,0.15)':'none',
                  }}
                />
                <AnimatePresence>
              ...
              <AnimatePresence mode="wait">
                {status==='success' && (
                  ...
                  >
                    ...
                    <span style={{color:'#E4F141',fontSize:'0.84rem',fontWeight:600,textShadow:'0 0 10px rgba(228,241,65,0.5)'}}>Message sent. We'll be in touch soon.</span>
                  </motion.div>
                )}
                {status==='error' && (
              ...
              <motion.button type="submit" disabled={loading}
                ...
                style={{
                  ...
                  background:loading?'rgba(228,241,65,0.3)':'linear-gradient(135deg,#d4e130 0%,#E4F141 50%,#d4e130 100%)',
                  border:'none', borderRadius:'13px', color:'#000',
                  fontSize:'0.9rem', fontWeight:800, cursor:loading?'not-allowed':'pointer',
                  letterSpacing:'0.05em', boxShadow:'0 8px 26px rgba(228,241,65,0.35)',
        ...
        >
          <div className="contact-map-header" style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'1.25rem', flexWrap:'wrap', gap:'0.75rem' }}>
            ...
            <motion.a
              ...
              whileHover={{ scale:1.04, background:'rgba(228,241,65,0.2)' }}
              whileTap={{ scale:0.97 }}
              style={{ display:'flex', alignItems:'center', gap:'7px', padding:'0.5rem 1.1rem', background:'rgba(228,241,65,0.12)', border:'1px solid rgba(228,241,65,0.4)', borderRadius:'999px', color:'#E4F141', fontSize:'0.75rem', fontWeight:700, textDecoration:'none', letterSpacing:'0.05em', transition:'all 0.2s',textShadow:'0 0 8px rgba(228,241,65,0.5)' }}
