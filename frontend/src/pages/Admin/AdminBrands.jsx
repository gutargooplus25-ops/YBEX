...
function AddBrandModal({ open, onClose, onAdd }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      onAdd(data.brand);
      onClose();
    } catch (e) { setErr(e.response?.data?.message || 'Failed to add brand'); }
  ...
  return (
    <AnimatePresence>
      <motion.div key="bd" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        ...
        style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.88)', backdropFilter: 'blur(16px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}
      >
        <motion.div key="bx"
          initial={{ opacity: 0, scale: 0.86, y: 48 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.86, y: 48 }}
          transition={{ duration: 0.42, ease: [0.22, 1, 0.36, 1] }}
          style={{ width: '100%', maxWidth: '500px', background: '#0d0d0d', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '24px', overflow: 'hidden', boxShadow: '0 60px 120px rgba(0,0,0,0.95)' }}
        >
          <div style={{ padding: '1.6rem 1.8rem 1.3rem', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'linear-gradient(135deg,rgba(228,241,65,0.06) 0%,transparent 55%)' }}>
            ...
            <motion.button whileHover={{ scale: 1.12 }} whileTap={{ scale: 0.9 }} onClick={onClose}
              style={{ width: '32px', height: '32px', borderRadius: '9px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.5)', cursor: 'pointer', fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
            >✕</motion.button>
          </div>
          ...
          <div style={{ padding: '1.6rem 1.8rem', display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
            <AnimatePresence>
              {err && (
                <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  style={{ background: 'rgba(255,61,16,0.08)', border: '1px solid rgba(255,61,16,0.28)', borderRadius: '10px', padding: '0.7rem 1rem', color: '#FF3D10', fontSize: '0.8rem' }}
                >⚠ {err}</motion.div>
              )}
            </AnimatePresence>
            ...
            <div>
              <label style={{ display: 'block', color: MUTED, fontSize: '0.67rem', fontWeight: 700, marginBottom: '0.5rem', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Brand Name <span style={{ color: ACCENT }}>*</span></label>
              <input type="text" placeholder="e.g. Nike, Colgate, Amazon" value={name} onChange={(e) => setName(e.target.value)} style={inp} onFocus={focus} onBlur={blur} />
            </div>

            <div>
              <label style={{ display: 'block', color: MUTED, fontSize: '0.67rem', fontWeight: 700, marginBottom: '0.5rem', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Website Link <span style={{ color: ACCENT }}>*</span></label>
              <input type="url" placeholder="https://brand.com" value={websiteLink} onChange={(e) => setWebsite(e.target.value)} style={inp} onFocus={focus} onBlur={blur} />
            </div>
          </div>
...
function BrandCard({ brand, index, onDelete }) {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      onDelete(brand._id);
    } catch (e) { console.error(e); setDeleting(false); setConfirmOpen(false); }
  };
  ...
  return (
    <>
      <DeleteConfirmModal
        ...
        loading={deleting}
        title="Delete Brand"
        message={`"${brand.name}" will be permanently removed from the homepage marquee.`}
      />
      ...
      <motion.div
        ...
        style={{ background: 'linear-gradient(145deg,rgba(255,255,255,0.045) 0%,rgba(255,255,255,0.02) 100%)', border: '1px solid ' + BORDER, borderRadius: '20px', overflow: 'hidden', position: 'relative', transition: 'border-color .3s' }}
      >
        <motion.div animate={{ opacity: hovered ? 1 : 0 }} transition={{ duration: 0.3 }}
          style={{ position: 'absolute', inset: 0, pointerEvents: 'none', borderRadius: '20px', background: 'radial-gradient(ellipse at 50% 0%,rgba(228,241,65,0.08) 0%,transparent 65%)' }} />

        <div style={{ width: '100%', height: '148px', background: 'rgba(255,255,255,0.03)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', position: 'relative', borderBottom: '1px solid ' + BORDER }}>
          {brand.logoUrl
            ? <motion.img src={brand.logoUrl} alt={brand.name}
                animate={{ scale: hovered ? 1.07 : 1 }}
                transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }}
                style={{ maxWidth: '74%', maxHeight: '104px', objectFit: 'contain', filter: hovered ? 'brightness(1.12) drop-shadow(0 4px 18px rgba(228,241,65,0.22))' : 'brightness(0.88)', transition: 'filter .3s' }}
              />
            : <div style={{ fontSize: '1.5rem', opacity: 0.18, color: '#fff' }}>NO LOGO</div>
          }
          <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'linear-gradient(to bottom,transparent 50%,rgba(0,0,0,0.32) 100%)' }} />
        </div>

        <div style={{ padding: '0.95rem 1.1rem 1.05rem' }}>
          <div style={{ fontSize: '0.88rem', fontWeight: 800, color: '#fff', marginBottom: '5px', letterSpacing: '-0.01em', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{brand.name}</div>
          <a href={brand.websiteLink} target="_blank" rel="noreferrer"
            style={{ fontSize: '0.67rem', color: ACCENT, textDecoration: 'none', fontWeight: 600, letterSpacing: '0.04em', opacity: 0.78, transition: 'opacity .2s' }}
        ...
        {/* Delete button */}
        <motion.button
          whileHover={{ scale: 1.15, background: 'rgba(255,61,16,0.35)' }}
          whileTap={{ scale: 0.9 }}
          onClick={() => setConfirmOpen(true)}
          style={{ position: 'absolute', top: '10px', right: '10px', width: '28px', height: '28px', background: 'rgba(0,0,0,0.58)', border: '1px solid rgba(255,61,16,0.25)', borderRadius: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.75rem', opacity: hovered ? 1 : 0.5, transition: 'opacity .2s, background .2s', color: '#ff6b35' }}
        >🗑</motion.button>

        <div style={{ position: 'absolute', top: '10px', left: '10px', background: 'rgba(0,0,0,0.62)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '6px', padding: '2px 7px', fontSize: '0.59rem', color: 'rgba(255,255,255,0.28)', fontWeight: 700 }}>#{index + 1}</div>
      </motion.div>
    </>
  );
}
...
export default function AdminBrands() {
  ...
  return (
    <AdminLayout>
      <AddBrandModal open={modal} onClose={() => setModal(false)} onAdd={(b) => setBrands((p) => [b, ...p])} />
      ...
      >
        💡 Paste any image URL (Google Images, Unsplash, CDN, etc.) as the logo. Brands appear on the homepage scrolling marquee.
      </motion.div>
      ...
      {loading ? (
        ...
        >
          <AnimatePresence>
            {brands.map((b, i) => (
              <BrandCard key={b._id} brand={b} index={i} onDelete={(id) => setBrands((p) => p.filter((x) => x._id !== id))} />
            ))}
          </AnimatePresence>
        </motion.div>
      )}
